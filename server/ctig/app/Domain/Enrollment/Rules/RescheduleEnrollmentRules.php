...
class RescheduleEnrollmentRules{
    ...
    public function execute(Enrollment $enrollment, Exam $toExam){
        ...
        $this->examGuard->ensureNotFinished($enrollment->exam, 'Запись нельзя перенести с прошедшего экзамена');
        $this->examGuard->ensureNotCancelled($toExam, 'Запись нельзя перенести на отмененный экзамен');
        $this->examGuard->ensureNotFinished($toExam, 'Запись нельзя перенести на прошедший экзамен');
